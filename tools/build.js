...
const sitemap = task('sitemap', () => {
  const urls = require('../src/routes.json')
    ...
    .map(x => ({ loc: x.path }));
  const template = fs.readFileSync('./www/sitemap.ejs', 'utf8');
  const render = ejs.compile(template, { filename: './www/sitemap.ejs' });
  const output = render({ config, urls });
  fs.writeFileSync('www/sitemap.xml', output, 'utf8');
});
